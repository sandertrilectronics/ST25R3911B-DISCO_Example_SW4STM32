...
static void rfalErrorHandling( void )
{
    ...
    if( gRFAL.conf.eHandling == RFAL_ERRORHANDLING_EMVCO )
    {
        ...
        if( (rfalFIFOStatusIsIncompleteByte() || rfalFIFOStatusIsMissingPar()) && (fifoBytesToRead > 0) )
        {
            fifoBytesToRead--;
        }
    ...
    switch( gRFAL.TxRx.state )
    {
        ...
        case RFAL_TXRX_STATE_RX_READ_DATA:
            ...
            if( !(gRFAL.TxRx.ctx.flags & RFAL_TXRX_FLAGS_CRC_RX_KEEP) )
            {
                ...
                if( st25r3911IsCRCinFIFO() && ((gRFAL.fifo.bytesTotal + tmp) > 0) )
                {
...
/*******************************************************************************/
ReturnCode rfalWakeUpModeStart( rfalWakeUpConfig *config )
{
    uint8_t                aux;
    uint8_t                reg;
    uint32_t               irqs;
    ...
    reg |= ((gRFAL.wum.cfg.period < RFAL_WUM_PERIOD_100MS) ? ST25R3911_REG_WUP_TIMER_CONTROL_wur : 0x00);
